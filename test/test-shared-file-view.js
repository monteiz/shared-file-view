...
describe("shared-file-view", function () {
	...
	describe("Writer", function () {
		beforeEach(function () {
			this.shobj = new SharedFileView.Create(path.join(this.dir, this.currentTest.title));
		});
		...
		it("must be called as a constructor", function () {
			const dir = this.dir;
			expect(function () {
				const obj = SharedFileView.Create(path.join(dir, "non-constructor"));
		...
		it("throws when writing after close", function () {
			const obj = new SharedFileView.Create(path.join(this.dir, "closetest-write"));
			obj["first"] = "value";
		...
		it("throws when deleting after close", function () {
			const obj = new SharedFileView.Create(path.join(this.dir, "closetest-del"));
			obj["first"] = "value";
			expect(obj.isClosed()).to.be.false;
			expect(obj.isOpen()).to.be.true;
			obj.close();
		...
		it("grows small files", function () {
			...
			const smallobj = new SharedFileView.Create(filename, 500);
		...
		it("throws exception when file gets too big", function () {
			...
			const smallobj = new SharedFileView.Create(filename, 40, 20, 40);
			let i;
			expect(function () {
				for (i = 0; i < 20; i++) {
		...
		it("avoids getting too big when rewriting the same key over and over", function () {
			const filename = path.join(this.dir, "bomb_me");
			const smallobj = new SharedFileView.Create(filename, 2, 2, 2);
	...
	describe("Informational methods:", function () {
		before(function () {
			this.obj = new SharedFileView.Create(path.join(this.dir, "free_memory_file"));
		});
		...
		it("has bucket_count", function () {
			this.obj = new SharedFileView.Create(path.join(this.dir, "bucket_counter"), 5, 4);
	...
	describe("Opener", function () {
		before(function () {
			...
			const writer = new SharedFileView.Create(this.testfile);
			...
			writer[this.bigKey] = new Array(BiggerKeySize).join(
				"six hundred seventy nine thousand nine hundred thirty two bytes long"
			);
			...
			writer.close();
			this.reader = new SharedFileView.Open(this.testfile);
		});

		it("must be called as a constructor", function () {
			const testfile = this.testfile;
			expect(function () {
				const obj = SharedFileView.Open(testfile);
				expect(obj).to.not.exist;
		...
		it("works across copies", function () {
			...
			const newReader = new SharedFileView.Open(newfile);
		...
		it("throws exception on non-existing file", function () {
			expect(function () {
				const obj = new SharedFileView.Open("/tmp/no_file_at_all");
				expect(obj).to.not.exist;
		...
		it("throws exception on a zero-length file", function () {
			...
			expect(function () {
				const reader = new SharedFileView.Open(newfile);
		...
		it("read after close gives exception", function () {
			const obj = new SharedFileView.Open(this.testfile);
		...
		it("throws exception on non-file file", function () {
			expect(function () {
				const obj = new SharedFileView.Open("/dev/null");
				expect(obj).to.not.exist;
		...
		it("throws exception on bad file", function () {
			expect(function () {
				const obj = new SharedFileView.Open(path.join(__dirname, "..", "testdata", "badfile.bin"));
				expect(obj).to.not.exist;
		...
		it("throws exception on another bad file", function () {
			...
			expect(function () {
				const obj = new SharedFileView.Open(path.join(__dirname, "..", "testdata", "badfile2.bin"));
				expect(obj).to.not.exist;
	...
	describe("Object comparison", function () {
		before(function () {
			...
			this.writer1 = new SharedFileView.Create(testfile1);
			this.writer1["first"] = "value for first";
			const testfile2 = path.join(this.dir, "prototest2");
			this.writer2 = new SharedFileView.Create(testfile2);
			this.writer2["first"] = "value for first";
			this.reader1 = new SharedFileView.Open(testfile1);
			this.reader2 = new SharedFileView.Open(testfile2);
		});
